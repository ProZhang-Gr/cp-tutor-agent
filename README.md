...
**在线体验**：<https://cp-tutor-agent.onrender.com>　|　**管理后台**：<https://cp-tutor-agent.onrender.com/admin>　|　**源码**：<https://github.com/ProZhang-Gr/cp-tutor-agent>

> 校企实训选题《项目 05 · 编程竞赛辅导智能体》的进阶 Web 实现。
> 首次打开若较慢，是免费云服务休眠后的冷启动（约 50 秒），稍候即可。

`FastAPI` · `LangGraph` · `LangChain` · `DeepSeek` · `真值判题` · `差分对拍` · `Python/C++` · `ReAct 调试` · `代码审阅` · `算法图解` · `社群` · `运营后台` · `中英双语` · `Monaco` · `Docker`
...
- **多语言**：编辑器与判题支持 **Python / C++**，C++ 为**真编译真运行**（g++，「编译一次、复用运行」以适配免费实例算力）；切换语言自动换语法高亮与代码模板。
...
生成 AI 日报后可**一键生成精美分享卡片图**（客户端 Canvas 绘制，平台无关、零授权），保存或复制后直接发到微信 / QQ / 朋友圈。卡片自带**指向站点的二维码**（内置离线二维码库，扫码即体验）+ **每日随机鼓励语**（按日期取种子，当天固定、跨天变化）+ **站点网址**，分享出去即是一张完整的引流名片。卡片自带**指向站点的二维码**（内置离线二维码库，扫码即体验）与**每日随机鼓励语**（按日期取种子，当天固定、跨天变化）。
...
- **按热度排序**：帖子列表默认按**点赞数排序**（🔥 最热），一键切「🕒 最新」，让真正有价值的讨论沉淀到前面；
- **题解关联做满**：题目页常驻「✎ 写本题题解」入口（一键跳社群并自动关联当前题目），题目下方聚合「这道题的社群讨论」（按热度），帖子详情可一键「去做这题」——做题 ↔ 复盘成回路；关联题目无法载入（私有 / 已删除）时给出明确提示而非静默失败；
...
- **换题即清场**：切题/换题时**智能体团队运行状态一并归零**（卡片状态 + 迷你状态点 + 进度条），不让上一题跑出来的「完成 ✓」误导当前题；判题结果、导师审阅、对话同步清空；
- **登录态一致**：登录 / 退出 / 自动退出后，仪表盘、社群、题单、日报区**统一刷新**，杜绝「顶栏已登录、某区域还显示请先登录」的矛盾状态；
- **闲置自动退出**：登录后 **10 分钟无操作自动退出登录**（任意交互续期），保护公共/共享设备上的账号安全；
...
- **平台激励闭环**：每日签到、社群发帖 / 答疑可得算力点（复用 credits、按日限次防刷）；攻克题目达到里程碑解锁**成就徽章**与可分享的**结业证书**（Canvas 绘制，带站点二维码）；
- **找回密码**：忘记密码可在登录页「向管理员申请重置」，由管理员后台核实身份后重置（详见下文「管理后台」，不接邮件、零外部依赖）；
- **防滥用**：IP 限流 + 每日配额 + 审计日志 + 输入上限 + 对话话题守门（给 API 花费封顶）；
- **数据持久层**：SQLAlchemy 2.0 ORM + 连接池 + Alembic 迁移，本地 SQLite / 线上 PostgreSQL(Neon) 透明切换。

### 11. 管理后台 · 运营控制台 🛠️
独立于前台的**管理员后台**，入口 **`/admin`**（线上 <https://cp-tutor-agent.onrender.com/admin>），深色「运营控制台」界面，登录态与普通用户**完全隔离**：

- **运营仪表盘**：用户总数 / 今日活跃 / 累计提交 / AI 调用量 / 学习时长等关键指标，配系统健康、近 14 天提交·调用趋势图与实时活动流（力求"有用而非堆砌"）；
- **用户管理**：用户增删改查、算力点调整、**重置密码**；密码列只展示哈希指纹（pbkdf2 单向，**不可逆、绝不显示明文**）；
- **找回密码工单**：处理用户提交的「申请重置密码」，核实身份后一键重置（带未读红点提醒）；
- **活动审计 / 内容治理**：可追溯平台调用记录（含 IP / 端点 / 指纹），下架违规帖子；
- **安全**：独立 HMAC 管理态（12h 自动失效）+ **登录失败锁定**（同 IP 连续失败即临时锁定）+ 登录审计留痕；账号通过环境变量 `ADMIN_USER` / `ADMIN_PASS` 配置，**默认 `manager` / `123456` 仅供本地，生产环境务必修改**。

### 12. 国际化 · 个性化 · 新手友好 🌐
- **中英双语**：顶栏一键切换 中 / English，界面文案即时本地化（题面与 AI 输出等内容数据保持原文，不二次翻译）；
- **三档主题**：右上角在 舒适（暖）/ 白天（白）/ 夜间（黑）间一键切换；
- **新手引导**：首次进入弹出**可跳过的聚光式引导**，分步介绍选题、分析、写码、判题、导师等核心操作；
- **系统公告**：顶栏公告中心，公测等通知一站触达、未读自动提醒。
...
| 前端 | 原生 **JS** + **Monaco** + **Chart.js** + **marked** + **Canvas** + **qrcode-generator** | 无框架、零构建；Monaco 编辑器、Chart.js 图表、marked 渲染 Markdown；算法图解与分享卡片用原生 Canvas/SVG 手绘；二维码用内置离线库 `qrcode-generator`（纯前端、不污染画布、可正常导出/复制） |
...
│   ├── admin.py        管理后台：鉴权 + 运营数据 + 用户增删改查 + 登录锁定
│   ├── support.py      找回密码工单（向管理员申请重置）
│   ├── telemetry.py    学习行为聚合埋点（时长/计数，非键鼠记录）
...
│                        + i18n.js 中英双语 + guide.js 新手引导/公告
│                        + admin.html/admin.js/admin.css 管理后台
│                        + qrcode.min.js 离线二维码库）
├── 宣传海报/            宣传海报（竖版 / 功能全景 / 五智能体，含去二维码版）
├── SECURITY.md         安全报告（全栈安全审计与加固记录）
...
4. 在 **Environment** 添加变量：
   ...
   - `SECRET_KEY = 任意随机串`（建议，签名登录态与管理态 token）
   - `ADMIN_USER` / `ADMIN_PASS = 管理后台账号口令`（**强烈建议**，覆盖公开仓库里的默认弱口令）
...
- **密码安全**：pbkdf2-HMAC-SHA256 加盐哈希，不存明文（**不可逆**，连管理后台也只见哈希指纹）；登录态用 HMAC 签名 Cookie，防篡改。
- **管理后台**：独立 HMAC 管理态（12h 失效）+ 登录失败锁定 + 操作审计；默认口令仅供本地，**生产用 `ADMIN_USER` / `ADMIN_PASS` 覆盖**。
- 更完整的全栈安全审计与加固记录见 [SECURITY.md](SECURITY.md)。
