...
/* 学习行为埋点（聚合，非键鼠记录）：只攒「专注秒数 + 操作计数」，定期/换题/离开时上报 */
let tActive = 0, tKeys = 0, tRuns = 0, tSubmits = 0;   // 本段累计
let tProblemId = "";                                    // 本段绑定的题目
let lastActivityTs = Date.now();                        // 最近一次用户交互时刻（判定是否「专注」）
let lastStats = null;                                   // 最近一次 /api/stats，供画像/成就复用
...
/* 智能体定义：node 名 -> 卡片（name/role 双语，随语言切换重贴标签） */
const AGENTS = [
  { id: "retrieve", icon: "🔍", name: "题库检索官", role: "RAG 相似题检索", nameEn: "Retriever", roleEn: "RAG similar-problem search" },
  { id: "analyze",  icon: "🧠", name: "题目分析师", role: "题型/难度/突破口", nameEn: "Analyst", roleEn: "Type / difficulty / insight" },
  { id: "plan",     icon: "🗺️", name: "策略规划师", role: "多解法 + 复杂度", nameEn: "Planner", roleEn: "Strategies + complexity" },
  { id: "tutor",    icon: "🎓", name: "苏格拉底导师", role: "分层提示 / 答疑", nameEn: "Socratic Tutor", roleEn: "Layered hints / Q&A" },
  { id: "review",   icon: "🔬", name: "代码审查师", role: "bug 定位 + 优化", nameEn: "Reviewer", roleEn: "Bug-finding + tuning" },
  { id: "test",     icon: "⚖️", name: "判题官", role: "真值判定 / 差分对拍", nameEn: "Judge", roleEn: "Ground-truth / stress test" },
  { id: "debug",    icon: "🛠️", name: "调试工程师", role: "ReAct 实验定位 bug", nameEn: "Debugger", roleEn: "ReAct bug localization" },
];
const NODE2AGENT = { retrieve:"retrieve", analyze:"analyze", plan:"plan",
                     review:"review", judge:"test", summarize:null };
const isEn = () => window.I18N && window.I18N.get() === "en";
const aName = (a) => isEn() ? a.nameEn : a.name;
const aRole = (a) => isEn() ? a.roleEn : a.role;
const aState = (s) => isEn() ? ({ working: "Working", done: "Done ✓", idle: "Idle" }[s] || "Idle")
                              : ({ working: "工作中", done: "完成 ✓", idle: "待命" }[s] || "待命");
...
function initAgentTeam() {
  $("#agent-team").innerHTML = AGENTS.map(a => `
    <div class="agent" id="agent-${a.id}">
      ...
      <div class="a-info"><div class="a-name">${aName(a)}</div><div class="a-role">${aRole(a)}</div></div>
      <div class="a-state">${aState("idle")}</div>
    </div>`).join("");
  $("#team-dots").innerHTML = AGENTS.map(a =>
    `<span class="team-dot" id="dot-${a.id}" title="${aName(a)}">${a.icon}</span>`).join("");
}
// 语言切换：只重贴名称/职责文案，不动各智能体的运行状态
function relabelAgents() {
  AGENTS.forEach(a => {
    const card = $(`#agent-${a.id}`);
    if (card) {
      const nm = card.querySelector(".a-name"), rl = card.querySelector(".a-role");
      if (nm) nm.textContent = aName(a);
      if (rl) rl.textContent = aRole(a);
      const st = card.querySelector(".a-state");
      if (st) {
        const cur = card.classList.contains("done") ? "done"
          : card.classList.contains("working") ? "working" : "idle";
        st.textContent = aState(cur);
      }
    }
    const dot = $(`#dot-${a.id}`); if (dot) dot.title = aName(a);
  });
}
function setAgent(id, state) {
  ...
  if (el) {
    ...
    if (state === "working") { el.classList.add("working"); label.textContent = aState("working"); }
    else if (state === "done") { el.classList.add("done"); label.textContent = aState("done"); }
    else { label.textContent = aState("idle"); }
  }
  const dot = $(`#dot-${id}`);
  if (dot) {
...
async function submitCode() {
  if (!editor) return;
  tSubmits++; lastActivityTs = Date.now();
  const code = editor.getValue();
  const problem = $("#problem-input").value.trim();
...
let currentCheckin = { today: false, points: 5 };   // 当日签到状态
...
async function loadMe() {
  try {
    ...
    currentCheckin = r.checkin || { today: false, points: 5 };
    window.currentUser = currentUser;   // 暴露给 community.js（let 绑定不会挂到 window 上）
    renderAuthArea(); renderAdaptiveNote();
    // 仪表盘若开着，登录态/算力点变化后刷新画像与成就（不必整页重载）
    if ($("#view-dashboard").classList.contains("active")) { renderProfilePanel(); renderAchievements(); }
...
// tier 档位标签随语言切换
const TIER_EN = { novice: "Novice", intermediate: "Intermediate", expert: "Expert" };
function tierLabelOf(p) {
  return isEn() ? (TIER_EN[p.tier] || p.tier_label || "Novice") : (p.tier_label || "新手");
}
function unitPt() { return isEn() ? "" : "点"; }
function renderAuthArea() {
  const a = $("#auth-area");
  if (currentUser) {
    const p = currentProfile || {}, b = currentBilling || {};
    const ptsTxt = isEn() ? `${b.credits || 0} cr` : `${b.credits || 0} 点`;
    const member = b.is_pro
      ? `<span class="pro-badge" title="算力点 ${b.credits}">PRO · ${ptsTxt}</span>`
      : `<span class="credits-chip">${ptsTxt}</span>`;
    const ci = currentCheckin || {};
    const ciCls = ci.today ? "checkin-btn done" : "checkin-btn";
    const ciTxt = ci.today ? t("checkin.done") : t("checkin.btn");
    a.innerHTML = `<span class="user-chip">
        <span class="user-name">${esc(currentUser.username)}</span>
        <span class="tier-badge ${p.tier || "novice"}" title="${esc(p.summary || "")}">${esc(tierLabelOf(p))}</span>
        ${member}
      </span><button class="${ciCls}" id="checkin-btn"${ci.today ? " disabled" : ""}>${ciTxt}</button><button class="ad-link" id="ad-btn" data-i18n-title="top.adLink.title" title="${t("top.adLink.title")}">${t("top.adLink")}</button><button class="recharge-link" id="recharge-btn">${t("top.recharge")}</button><button class="logout-btn" id="logout-btn">${t("top.logout")}</button>`;
    $("#logout-btn").onclick = logout;
    $("#recharge-btn").onclick = () => $("#recharge-modal").classList.remove("hidden");
    $("#ad-btn").onclick = openAdModal;
    const cb = $("#checkin-btn"); if (cb && !ci.today) cb.onclick = doCheckin;
  } else {
    a.innerHTML = `<button class="auth-btn ghost" data-open="login">${t("auth.login")}</button>
                   <button class="auth-btn" data-open="register">${t("auth.register")}</button>`;
...
async function doCheckin() {
  try {
    ...
    const data = await r.json();
    if (!r.ok) { toast(data.error || (isEn() ? "Check-in failed" : "签到失败")); if (r.status === 409) { currentCheckin.today = true; renderAuthArea(); } return; }
    currentCheckin.today = true;
    await loadMe();
    toast(isEn() ? `✅ Checked in · +${data.gained} credits` : `✅ 签到成功 · +${data.gained} 算力点`);
  } catch (e) { toast(isEn() ? "Check-in failed" : "签到失败"); }
}
function renderAdaptiveNote() {
  ...
  if (currentUser && currentProfile && !currentProfile.is_empty) {
    ...
    if (isEn()) {
      n.innerHTML = `Adaptive tutoring tuned to your <b>${esc(tierLabelOf(p))}</b> tier` +
        (p.weak_types && p.weak_types.length ? ` (weak: ${esc(p.weak_types.join(", "))})` : "");
    } else {
      n.innerHTML = `已按你的画像 <b>${esc(tierLabelOf(p))}档</b> 因材施教` +
        (p.weak_types && p.weak_types.length ? `（薄弱：${esc(p.weak_types.join("、"))}）` : "");
    }
...
(function bindIdleActivity() {
  ...
  const onActivity = () => {
    const now = Date.now();
    lastActivityTs = now;        // 喂给学习埋点：判定「专注」用
...
/* ====================================================================
   用户画像 · 学习投入 · 成就证书（仪表盘新增三块）
==================================================================== */
function chipList(arr, cls) {
  return (arr && arr.length)
    ? arr.map(x => `<span class="pf-chip ${cls || ""}">${esc(x)}</span>`).join("")
    : `<span class="pf-chip muted">${isEn() ? "—" : "暂无"}</span>`;
}
function renderProfilePanel() {
  const box = $("#profile-body");
  if (!box) return;
  const p = currentProfile || {};
  if (!currentUser || p.is_empty) {
    box.innerHTML = `<div class="pf-empty">${t("profile.empty")}</div>`;
    return;
  }
  const L = (k) => t(k);
  const mastery = (lastStats && lastStats.type_mastery) || [];
  const masteryHtml = mastery.length ? mastery.slice(0, 6).map(m => `
    <div class="pf-mrow">
      <span class="pf-mname">${esc(m.type)}</span>
      <div class="pf-mbar"><i style="width:${m.rate}%"></i></div>
      <span class="pf-mpct">${m.rate}%<span class="pf-mcnt"> · ${m.count}${isEn() ? "" : "题"}</span></span>
    </div>`).join("") : `<div class="pf-empty-sm">${isEn() ? "Not enough data yet" : "数据还不够"}</div>`;
  box.innerHTML = `
    <div class="pf-top">
      <div class="pf-tier"><span class="tier-badge ${p.tier || "novice"}">${esc(tierLabelOf(p))}</span></div>
      <div class="pf-kv"><span class="pf-k">${L("profile.solveRate")}</span><span class="pf-v">${p.solve_rate || 0}%</span></div>
      <div class="pf-kv"><span class="pf-k">${L("dash.st.solved")}</span><span class="pf-v">${p.solved || 0}/${p.total || 0}</span></div>
    </div>
    <div class="pf-row"><span class="pf-label">${L("profile.strong")}</span>${chipList(p.strong_types, "good")}</div>
    <div class="pf-row"><span class="pf-label">${L("profile.weak")}</span>${chipList(p.weak_types, "warn")}</div>
    <div class="pf-row"><span class="pf-label">${L("profile.errors")}</span>${chipList(p.common_errors, "")}</div>
    <div class="pf-mastery"><div class="pf-label">${L("profile.mastery")}</div>${masteryHtml}</div>`;
}

/* 成就徽章：从 stats 推导里程碑（纯客户端，可解释） */
const CERT_MIN = 3;   // 至少攻克 3 题才解锁结业证书
const ACHIEVEMENTS = [
  { id: "first", icon: "🎯", zh: "首次 AC", en: "First AC", dzh: "通过第 1 道题", den: "Solve your 1st problem", ok: (s) => s.solved >= 1 },
  { id: "solve5", icon: "🔥", zh: "小有所成", en: "Getting Started", dzh: "累计攻克 5 题", den: "Solve 5 problems", ok: (s) => s.solved >= 5 },
  { id: "solve20", icon: "🏆", zh: "刷题达人", en: "Problem Slayer", dzh: "累计攻克 20 题", den: "Solve 20 problems", ok: (s) => s.solved >= 20 },
  { id: "active7", icon: "📆", zh: "坚持一周", en: "7 Active Days", dzh: "活跃满 7 天", den: "Active for 7 days", ok: (s) => (s.active_days || 0) >= 7 },
  { id: "rate80", icon: "🎖️", zh: "稳准狠", en: "Sharpshooter", dzh: "≥5 题且通过率≥80%", den: "≥5 solved & ≥80% pass", ok: (s) => s.total >= 5 && s.solve_rate >= 80 },
  { id: "submit50", icon: "⚙️", zh: "千锤百炼", en: "Grinder", dzh: "累计提交 50 次", den: "50 submissions", ok: (s) => (s.total || 0) >= 50 },
];
function renderAchievements() {
  const box = $("#ach-badges"), btn = $("#btn-gen-cert");
  if (!box) return;
  const s = lastStats || { solved: 0, total: 0, active_days: 0, solve_rate: 0 };
  box.innerHTML = ACHIEVEMENTS.map(a => {
    const ok = a.ok(s);
    return `<div class="ach-badge ${ok ? "got" : "locked"}" title="${esc(isEn() ? a.den : a.dzh)}">
        <span class="ach-ico">${a.icon}</span>
        <span class="ach-name">${esc(isEn() ? a.en : a.zh)}</span>
      </div>`;
  }).join("");
  if (btn) {
    const unlocked = currentUser && s.solved >= CERT_MIN;
    btn.disabled = !unlocked;
    btn.textContent = unlocked ? t("ach.cert") : t("ach.cert.locked");
  }
}

/* 学习投入卡片：拉聚合埋点填数 */
async function loadEngagementCard() {
  try {
    const s = await fetch("/api/telemetry/summary").then(r => r.json());
    const set = (id, v) => { const e = $(id); if (e) e.textContent = v; };
    set("#eg-today", s.today_minutes != null ? s.today_minutes : 0);
    set("#eg-total", s.total_minutes != null ? s.total_minutes : 0);
    set("#eg-keys", s.total_keystrokes != null ? s.total_keystrokes : 0);
    set("#eg-persub", s.avg_minutes_per_submit != null ? s.avg_minutes_per_submit : 0);
  } catch (e) {}
}

/* ---------------- 学习行为埋点采集器（聚合，非键鼠记录） ---------------- */
const TELEMETRY_FLUSH_MS = 30000;   // 每 30s 上报一段
function telemetryTick() {
  // 仅在「页面可见 且 30s 内有过交互」时计入专注时长，避免把挂机也算成学习
  if (document.visibilityState === "visible" && Date.now() - lastActivityTs < 30000) tActive++;
}
function telemetryPayload() {
  return { problem_id: currentProblemId || "", active_seconds: tActive,
           keystrokes: tKeys, runs: tRuns, submits: tSubmits };
}
function telemetryReset() { tActive = 0; tKeys = 0; tRuns = 0; tSubmits = 0; }
function telemetryFlush(useBeacon) {
  if (tActive === 0 && tKeys === 0 && tRuns === 0 && tSubmits === 0) return;
  ...
  try {
    if (useBeacon && navigator.sendBeacon) {
      ...
      fetch("/api/telemetry", { method: "POST", headers: { "Content-Type": "application/json" }, body, keepalive: true })
        .then(() => { if ($("#view-dashboard").classList.contains("active")) loadEngagementCard(); })
        .catch(() => {});
    }
  } catch (e) {}
}
function initTelemetry() {
  ...
  setInterval(() => telemetryFlush(false), TELEMETRY_FLUSH_MS);
  document.addEventListener("visibilitychange", () => { if (document.visibilityState === "hidden") telemetryFlush(true); });
  window.addEventListener("pagehide", () => telemetryFlush(true));
}

/* ---------------- 结业证书（客户端 Canvas，带二维码） ---------------- */
function openCertModal() {
  if (!currentUser) { openAuth("login", isEn() ? "Sign in to earn your certificate" : "登录后即可生成你的结业证书"); return; }
  if (!lastStats || lastStats.solved < CERT_MIN) { toast(t("ach.cert.locked")); return; }
  $("#cert-modal").classList.remove("hidden");
  drawCertificate();
}
function closeCertModal() { $("#cert-modal").classList.add("hidden"); }
async function drawCertificate() {
  const canvas = $("#cert-canvas");
  const SC = 2, W = 900, H = 640;
  canvas.width = W * SC; canvas.height = H * SC;
  const ctx = canvas.getContext("2d");
  ctx.scale(SC, SC);
  try { await document.fonts.ready; } catch (e) {}
  ...
  const ink = "#2E2A22", inkDim = "#6E6657", inkMute = "#9C9484",
        accent = "#1F6F66", gold = "#BE8E2C", paper = "#FBF7EF", line = "#E1D8C6", tint = "#F0E9D9";
  const s = lastStats || {}, en = isEn();
  const name = currentUser ? currentUser.username : "";
  const today = new Date().toISOString().slice(0, 10);

  ctx.fillStyle = "#EBE3D3"; ctx.fillRect(0, 0, W, H);
  roundRect(ctx, 22, 22, W - 44, H - 44, 20); ctx.fillStyle = paper; ctx.fill();
  // 双描边边框，证书感
  ctx.strokeStyle = accent; ctx.lineWidth = 2; ctx.stroke();
  roundRect(ctx, 34, 34, W - 68, H - 68, 14); ctx.strokeStyle = line; ctx.lineWidth = 1; ctx.stroke();

  ctx.textAlign = "center";
  // 品牌
  ctx.fillStyle = accent; roundRect(ctx, W / 2 - 26, 64, 52, 52, 14); ctx.fill();
  ctx.fillStyle = paper; ctx.font = "700 24px " + MONO; ctx.textBaseline = "middle";
  ctx.fillText("/A", W / 2, 91); ctx.textBaseline = "alphabetic";
  // 标题
  ctx.fillStyle = ink; ctx.font = "700 40px " + SERIF;
  ctx.fillText(en ? "Certificate of Completion" : "结 业 证 书", W / 2, 166);
  ctx.fillStyle = gold; ctx.font = "600 14px " + MONO;
  ctx.fillText("ARENA · " + (en ? "CP Tutor Agent" : "算法竞赛辅导智能体"), W / 2, 192);
  ctx.strokeStyle = line; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(W / 2 - 120, 210); ctx.lineTo(W / 2 + 120, 210); ctx.stroke();

  // 姓名
  ctx.fillStyle = inkMute; ctx.font = "400 15px " + SANS;
  ctx.fillText(en ? "This certifies that" : "兹 证 明", W / 2, 256);
  ctx.fillStyle = accent; ctx.font = "700 36px " + SERIF;
  ctx.fillText(name, W / 2, 300);

  // 正文成就句
  ctx.fillStyle = ink; ctx.font = "400 18px " + SANS;
  const body = en
    ? `has solved ${s.solved || 0} problems on ARENA with a ${s.solve_rate || 0}% pass rate,`
    : `在 ARENA 累计攻克 ${s.solved || 0} 道算法题，通过率 ${s.solve_rate || 0}%，`;
  const body2 = en
    ? `staying active for ${s.active_days || 0} days. Keep going!`
    : `活跃学习 ${s.active_days || 0} 天，特此证明，再接再厉！`;
  ctx.fillText(body, W / 2, 344);
  ctx.fillText(body2, W / 2, 372);

  // 三宫格统计
  const tiles = [
    { n: s.solved || 0, l: en ? "Solved" : "已攻克" },
    { n: (s.solve_rate || 0) + "%", l: en ? "Pass rate" : "通过率" },
    { n: s.active_days || 0, l: en ? "Active days" : "活跃天数" },
  ];
  const tw = 150, gap = 24, ty = 410, th = 92;
  const tx0 = W / 2 - (tw * 3 + gap * 2) / 2;
  tiles.forEach((tl, i) => {
    const tx = tx0 + i * (tw + gap);
    roundRect(ctx, tx, ty, tw, th, 14); ctx.fillStyle = tint; ctx.fill();
    ctx.strokeStyle = line; ctx.lineWidth = 1; ctx.stroke();
    ctx.fillStyle = accent; ctx.font = "700 34px " + SERIF;
    ctx.fillText(String(tl.n), tx + tw / 2, ty + 46);
    ctx.fillStyle = inkDim; ctx.font = "500 14px " + SANS;
    ctx.fillText(tl.l, tx + tw / 2, ty + 74);
  });

  // 底部：日期 + 二维码 + 落款
  ctx.textAlign = "left";
  ctx.fillStyle = inkDim; ctx.font = "600 14px " + MONO;
  ctx.fillText((en ? "Date " : "签发日期 ") + today, 70, H - 70);
  ctx.fillStyle = inkMute; ctx.font = "400 12px " + SANS;
  ctx.fillText(en ? "Verify & practice at cp-tutor-agent" : "扫码验证并开始练习", 70, H - 50);
  const SITE_URL = "https://cp-tutor-agent.onrender.com";
  drawQRCode(ctx, SITE_URL, W - 70 - 92, H - 70 - 70, 92, { dark: ink, light: paper, quiet: 6 });
  ctx.textAlign = "right";
  ctx.fillStyle = accent; ctx.font = "italic 600 20px " + SERIF;
  ctx.fillText("ARENA", W - 180, H - 60);
  ctx.textAlign = "left";
}
function downloadCert() {
  const canvas = $("#cert-canvas");
  canvas.toBlob((blob) => {
    if (!blob) return toast(isEn() ? "Failed" : "生成失败");
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "ARENA_Certificate_" + (currentUser ? currentUser.username : "") + ".png";
    a.click(); URL.revokeObjectURL(url);
    toast(isEn() ? "Saved" : "已保存图片");
  });
}
async function copyCert() {
  ...
  try {
    ...
    if (!blob || !navigator.clipboard || !window.ClipboardItem) throw new Error("unsupported");
    await navigator.clipboard.write([new ClipboardItem({ "image/png": blob })]);
    toast(isEn() ? "✅ Image copied" : "✅ 图片已复制");
  } catch (e) { toast(isEn() ? "Copy unsupported, use Save" : "此浏览器不支持复制图片，请用「保存图片」"); }
}
function bindCert() {
  const gen = $("#btn-gen-cert"); if (gen) gen.onclick = openCertModal;
  const cl = $("#cert-close"); if (cl) cl.onclick = closeCertModal;
  const dl = $("#cert-download"); if (dl) dl.onclick = downloadCert;
  const cp = $("#cert-copy"); if (cp) cp.onclick = copyCert;
  const m = $("#cert-modal"); if (m) m.onclick = (e) => { if (e.target.id === "cert-modal") closeCertModal(); };
}
